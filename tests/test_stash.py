...
def test_molior(cellcomplex, tmp_path):
    """Test Molior building initialization and file writing"""
    # Molior builds IFC models
    from molior import Molior

    molior = Molior(cellcomplex=cellcomplex, name="My Topologic building")
    molior.init_building()
    molior.stash_topology()

    # Use tmp_path for temporary file
    output_file = tmp_path / "_test.ifc"
    molior.file.write(str(output_file))

    # Verify the file was created
    assert output_file.exists()


def test_footprint(cellcomplex):
    """Test the footprint generation"""
    paths = cellcomplex.FootPrint()

    for path in paths:
        assert path.is_simple_cycle()
        assert len(path.graph) == 15

        for vertex in path.graph:
            assert len(topologist.helpers.string_to_coor(vertex)) == 3
